...
class CorruptionGenerationLayerTrain(tf.keras.layers.Layer):
    ...
    def call(self, pos, ent_size, eta):
        ...
        keep_subj_mask = tf.cast(tf.random.uniform(
            [tf.shape(input=dataset)[0]], 0, 2, dtype=tf.int32, seed=self.seed), tf.bool)
        ...
        # corruptions
        corruptions = tf.transpose(a=tf.stack(
            [subjects, relationships, objects]))
        return corruptions
