...
class EmbeddingModel(abc.ABC):
 ...
 Kindly change the optimizer and restart the experiment")
    ...
    def _calibrate_with_corruptions(self, dataset_handle, batch_size_pos, _):
        """
        Calibrates model with corruptions. The corruptions are hard-coded to be 's+o' with all available entities.

        Parameters
        ----------
        dataset_handle: AmpligraphDatasetAdapter
            Dataset adapter containing the positive triples.
        ...
        scores_neg: tf.Tensor
            Tensor with negative scores (generated by corruptions).
    ...
    def _calibrate_with_negatives(self, dataset_handle, batch_size_pos, batch_size_neg):
        """
        ...
        ----------
        dataset_handle: AmpligraphDatasetAdapter
            Dataset adapter containing the positive and negative triples.

        batch_size_pos: int
            Batch size of the positive triples dataset.

        batch_size_neg: int
            Batch size of the negative triples dataset.
        ...
        """
        pos_generator = partial(dataset_handle.get_next_train_batch, batch_size=batch_size_pos, dataset_type="pos")
        neg_generator = partial(dataset_handle.get_next_train_batch, batch_size=batch_size_neg, dataset_type="neg")

        dataset_pos = tf.data.Dataset.from_generator(pos_generator,
                                                     output_types=tf.int32,
                                                     output_shapes=(None, 3))

        dataset_neg = tf.data.Dataset.from_generator(neg_generator,
                                                     output_types=tf.int32,
                                                     output_shapes=(None, 3))

        choice_dataset = tf.data.Dataset.range(2).repeat()
        dataset = tf.data.experimental.choose_from_datasets([dataset_pos, dataset_neg], choice_dataset)

        dataset = dataset.repeat().prefetch(1)
        dataset_iter = tf.data.make_one_shot_iterator(dataset)

        x_pos_tf = dataset_iter.get_next()
        x_neg_tf = dataset_iter.get_next()
    ...
    def calibrate(self, X_pos, X_neg=None, positive_base_rate=None, batches_count=100, epochs=50):
        """Calibrates the predictions using [Platt scaling](https://en.wikipedia.org/wiki/Platt_scaling).

        The calibrated predictions can be obtained with `predict_proba` after the calibration is done.

        There are two modes of operation:

        1. Both positive and negative triples are provided via `X_pos` and `X_neg` respectively.
        2. Only positive triples are provided, and the negative triples are generated by corruptions
        just like it is done in training or evaluation.
        ...
        positive_base_rate: float
            ...
            the positive base rate is 50/(50+200) = 1/5 = 0.2.
