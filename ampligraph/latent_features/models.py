...
class ConvE(EmbeddingModel):
    ...
    def _fn(self, e_s, e_p, e_o):
        ...
        # x = self.dropout_input(x, training=not self.is_fitted)
    ...
    def _load_model_from_trained_params(self):
        ...
        if not self.dealing_with_large_graphs:
            ...
            self.dropout_input = tf.keras.layers.Dropout(rate=self.embedding_model_params['dropout_embed'])
            self.embed_image = tf.keras.layers.Reshape(target_shape=[self.embedding_model_params['embed_image_height'],
                                                                     ...
                                                                     1])
            # self.conv2d = tf.keras.layers.Conv2D.from_config(self.trained_model_params['conv2d_config'])
            
            # TODO: Check should trainable=True really be set here
            ...
            self.conv2d = tf.keras.layers.Conv2D(filters=self.embedding_model_params['conv_filters'], trainable=False,
                                                 ...
                                                 kernel_initializer=tf.keras.initializers.constant(
                                                     self.trained_model_params['conv2d'][0]),
                                                 bias_initializer=tf.keras.initializers.constant(
                                                     self.trained_model_params['conv2d'][1]))
 ...
 Kindly change the optimizer and restart the experiment")
    ...
    def _initialize_eval_graph(self, mode="test"):
        ...
        # TODO: Update _test_generator, maybe include scores_filter (?)
        dataset = tf.data.Dataset.from_generator(partial(self._test_generator, mode=mode),
                                                 output_types=(tf.int32, tf.float32),
                                                 output_shapes=((None, 3), (None, len(self.ent_to_idx))))
