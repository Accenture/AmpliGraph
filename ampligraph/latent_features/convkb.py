...
class ConvKB(EmbeddingModel):
    ...
    >>> model = ConvKB(batches_count=1, seed=555, epochs=20, k=10,
        ...
        if not self.dealing_with_large_graphs:
            ...
            for i, filter_size in enumerate(filter_sizes):
                ...
                self.conv_weights[conv_name] = {'weights': tf.get_variable('{}_W'.format(conv_name), shape=conv_shape,
                                                                          trainable=is_trainable, dtype=tf.float32,
                                                                          initializer=tf.initializers.truncated_normal(seed=self.seed)),
                                                'biases': tf.get_variable('{}_B'.format(conv_name), shape=[num_filters],
                                                                          trainable=is_trainable, dtype=tf.float32,
                                                                          initializer=tf.zeros_initializer())}

            self.dense_W = tf.get_variable('dense_weights', shape=[dense_dim, num_outputs], trainable=is_trainable,
                                           initializer=tf.keras.initializers.he_normal(seed=self.seed),
                                           dtype=tf.float32)
            self.dense_B = tf.get_variable('dense_bias', shape=[num_outputs], trainable=is_trainable,
