...
def discover_facts(X, model, top_n=10, strategy='random_uniform', max_candidates=.3, target_rel=None, inverse_weights=True, seed=0):
    """
    ...
    for relation in rel_list:
        ...
        # print('Generating candidates for relation: %s' % relation)

        candidates = generate_candidates(X_filtered, strategy, relation, max_candidates, inverse_weights=inverse_weights, seed=seed)

        logger.debug('Generated %d candidate statements.' % len(candidates))

        # Get ranks of candidate statements
        ranks = evaluate_performance(candidates, model=model, filter_triples=X, use_default_protocol=True, verbose=False)
...
def _filter_candidates(X_candidates, X):
    """
    Filter candidate statements from X_candidates that are in X.
    Also removes reflexive statements like: ['x', 'r', 'x']
    Parameters
    ----------
    X_candidates: A candidate set
    X: A reference set
...
def exhaustive_generator(X, target_rel, max_candidates, consolidate_sides=False, seed=0):
    ...
    # Exhaustive, generate all combinations of subject and object entities for target_rel
    batch_count, total_count, num_combn = 0, 0, len(e_s) * len(e_o)
    X_candidates = []

    for subj, obj in itertools.product(e_s, e_o):

        X_candidates.append([subj, target_rel, obj])
        batch_count += 1
        total_count += 1
        if batch_count == max_candidates or total_count == num_combn:
            X_candidates = _filter_candidates(X_candidates, X)
            yield np.array(X_candidates)
            X_candidates = []


def generate_candidates(X, strategy, target_rel, max_candidates, consolidate_sides=False, inverse_weights=True, seed=0):
    ...
    if strategy == 'random_uniform':
        ...
        num_retries, total_retries = 0, 5  # Retry up to 5 times to reach max_candidates
        X_candidates = np.empty([0, 3], dtype=object)

        while len(X_candidates) < max_candidates or num_retries < total_retries:
            ...
            X_candidates = np.append(X_candidates, gen_candidates, axis=0)
            num_retries += 1

        return X_candidates[:max_candidates, :]
