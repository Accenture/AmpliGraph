...
def discover_facts(X, model, top_n=10, strategy='exhaustive', max_candidates=.3,
                   target_rel=None, corruption_entities=None, max_negatives=100, seed=0):
    ...
    """ Discover new facts from an existing knowledge graph.
        ...
        model : EmbeddingModel
            The trained model that will be used to score candidate facts. ``model`` must have been
            previously trained on ``X``.
        top_n : int
            The cutoff position in ranking to consider a candidate triple as true positive.
        strategy: string
            The candidates generation strategy. Choose from 'exhaustive', 'random_uniform'.
        max_candidates: int or float
            The maximum numbers of candidates generated by 'strategy'. Can be an absolute number or a percentage [0,1].
        target_rel : str
        ...
        corruption_entities : ndarray, shape [n]
            A list of entities that will used in the evaluation step to generate negatives that will be ranked against
            candidate statements. If None, all entities will be used.
        max_negatives : int
            The maximum number of corrupted triples to generate during evaluation. The higher this value,
    ...
    for relation in rel_list:
        ...
        candidate_generator = generate_candidates(X_filtered, strategy, relation, max_candidates)
        ...
        for candidates in candidate_generator:
            ...
            # standard protocol evaluates against corruptions on both sides, we just average the ranks here
...
def generate_candidates(X, strategy, target_rel, max_candidates, consolidate_sides=False):
    """ Generate candidate statements from an existing knowledge graph using a defined strategy.
        ...
        strategy: string
            The candidates generation strategy. Choose from 'exhaustive', 'random_uniform'.
            - 'exhaustive' strategy generates all possible candidates given the ```target_rel``` and
    ...
    if target_rel not in relations:
        # Not raising error as not clear this would or should always be the case.
    ...
    if strategy == 'exhaustive':
        ...
        for ent in e_s:
            ...
            # NB: May want to consider filtering triples of form: <ent_id1, target_rel, ent_id1>, but
            # this seems less of a problem if considering a single relationship at time, and with
        ...
        # Take sqrt of max_candidates so that len(meshgrid result) == max_candidates
