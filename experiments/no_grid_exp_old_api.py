...
def main():
    ...
    try:
        model = model_class(batches_count=hyperparams["batches_count"], 
                            seed=hyperparams["seed"], 
                            epochs=hyperparams["epochs"], 
                            k=hyperparams["k"], 
                            pairwise_margin=hyperparams["pairwise_margin"],
                            lr=hyperparams["lr"], 
                            eta=hyperparams["eta"],
                            ...
                            optimizer='adagrad', 
                            verbose=True)
        print("initialized model with reg...{0} with params...{1}".format(model_class, args.hyperparams))
    except:
        model = model_class(batches_count=hyperparams["batches_count"], 
                            seed=hyperparams["seed"], 
                            epochs=hyperparams["epochs"], 
                            ...
                            optimizer='adagrad', 
                            verbose=True)
        print("initialized model with NO reg...{0} with params...{1}".format(model_class, args.hyperparams))
