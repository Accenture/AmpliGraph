...
def test_l1_regularizer():
    l1_class = REGULARIZER_REGISTRY['L1']
    p1 = tf.constant([1,-1,1], dtype=tf.float32)
    p2 = tf.constant([2,-2,2], dtype=tf.float32)
    params = [p1, p2]
    lambda_1 = 1.0
    lambda_2 = [2.0, 3.0]
    l1_obj1 = l1_class({'lambda': lambda_1})
    l1_obj2 = l1_class({'lambda': lambda_2})
...
def test_l2_regularizer():
    l2_class = REGULARIZER_REGISTRY['L2']
    p1 = tf.constant([1,-1,1], dtype=tf.float32)
    p2 = tf.constant([2,-2,2], dtype=tf.float32)
    params = [p1, p2]
    lambda_1 = 1.0
    lambda_2 = [2.0, 3.0]
    l2_obj1 = l2_class({'lambda': lambda_1})
    l2_obj2 = l2_class({'lambda': lambda_2})
