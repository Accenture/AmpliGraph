...
def test_PairwiseLoss():
    ...
    assert (loss.numpy() == np.array([2.0, 2.0], dtype=np.float32)).all(), \
    ...
    # Loss: 2, 2, 4, 4, 0, 0
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([6.0, 6.0], dtype=np.float32)).all(), \
        'PairwiseLoss: Loss function outputs dont match expected outputs'
...
def test_NLLLoss():
    ...
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([21.0, 10.0], dtype=np.float32)).all(), \
    ...
    # Loss: 51, 30, 0, 0, 75, 30
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([126.0, 60.0], dtype=np.float32)).all(), \
        'NLLLoss: Loss function outputs dont match expected outputs'
...
def test_AbsoluteMarginLoss():
    ...
    # Loss: 6, 10, 3, 10, 0, 10
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([3.0, 10.0], dtype=np.float32)).all(), \
    ...
    # Loss: 6, 10, 3, 10, 0, 10
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([9.0, 30.0], dtype=np.float32)).all(), \
        'NLLLoss: Loss function outputs dont match expected outputs'
...
def test_SelfAdversarialLoss():
    ...
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([1.3552092, 9.222016], dtype=np.float32)).all(), \
    ...
    corr_score = tf.constant([np.log(2), np.log(10), np.log(2), np.log(50), np.log(4), np.log(40)], dtype=tf.float32)
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([4.060676, 13.664226], dtype=np.float32)).all(), \
        'SelfAdversarialLoss: Loss function outputs dont match expected outputs'
...
def test_NLLMulticlass():
    ...
    loss = lossObj(pos_score, corr_score, eta=3)
    assert (loss.numpy() == np.array([1.3862944, 1.3862944], dtype=np.float32)).all(), \
    ...
    assert (loss.numpy() == np.array([2.3025851, 2.3025851], dtype=np.float32)).all(), \
        'NLLMulticlass: Loss function outputs dont match expected outputs'
